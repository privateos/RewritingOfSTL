...
#ifndef beauty__container__set___hash_set_hpp
#define beauty__container__set___hash_set_hpp
...
class hash_set{
	...
	hash_set(size_type n):t(n){}
	
	hash_set(HashFun &hf):t(hf){}
	
	hash_set(size_type n, HashFun &hf):t(n, hf){}
	
	hash_set(Compare &c, HashFun &hf):
	t(beauty::functor::identity<Key>(), c, hf){}
	
	hash_set(size_type n, Compare &c, HashFun &hf):
	...
	pair_iterator_bool insert(const value_type &v){
		return t.const_insert_unique(v);
	}
