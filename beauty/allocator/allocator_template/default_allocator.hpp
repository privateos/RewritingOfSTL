...
#ifndef beauty__allocator__allocator_template___default_allocator_hpp
#define beauty__allocator__allocator_template___default_allocator_hpp
...
class default_allocator{
			...
			if(bytes_left >= total_bytes){
				...
				if(bytes_left >= size){
					...
					if(start_free == 0){
						...
						for(i = size; i <= static_cast<size_t>(MAX_BYTES); i += size){
	...
	public:
		...
		static void* reallocate(void *p, size_t old_size, size_t new_size){
			if(old_size > static_cast<size_t>(MAX_BYTES) &&
			  new_size > static_cast<size_t>(MAX_BYTES)){
...
typedef default_allocator<false,0> def_alloc;
